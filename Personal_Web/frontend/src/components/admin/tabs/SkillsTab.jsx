...
const SkillsTab = ({ initialData, onUpdate }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (currentSkill.id) {
        // Update
        const res = await axios.put(`http://localhost:5000/api/admin/skills/${currentSkill.id}`, payload, config);
        const updatedSkills = skills.map(s => s.id === currentSkill.id ? res.data : s);
        setSkills(updatedSkills);
        onUpdate('skills', updatedSkills);
      } else {
        // Create
        const res = await axios.post(`http://localhost:5000/api/admin/skills`, payload, config);
