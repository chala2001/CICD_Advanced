...
const ExperienceTab = ({ initialData, onUpdate }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (currentExp.id) {
        // Update
        const res = await axios.put(`http://localhost:5000/api/admin/experience/${currentExp.id}`, currentExp, config);
        ...
        // Create
        const res = await axios.post(`http://localhost:5000/api/admin/experience`, currentExp, config);
